...
Intuitively, when a function $g$ approaches a nonzero limit $M$ near a
point, there exists an interval in which the values of $g$ are closer
to $M$ than to zero.

\vs

More formally, suppose $\lim_{x\to a}g(x)=M$. We show that there exists
some $\delta$ such that $0<|x-a|<\delta$ implies for all $x$:
...
Let's prove each of the three claims individually.
